...
foreach (${$pluralVar} as ${$singularVar}):
	echo '<tr>';
		...
		echo ' ' . $this->Form->postLink(
			...
			array(),
			__d('cake', 'Você deseja realmente deletar # %s?', ${$singularVar}[$modelClass][$primaryKey])
		);
		echo '</td>';
	echo '</tr>';
...
?>
	...
	echo $this->Paginator->counter(array(
		'format' => __d('cake', 'Página {:page} de {:pages}, exibindo {:current} de {:count} total, iniciando em {:start}, terminando em {:end}')
	));
	?></p>
	<div class="paging">
	<?php
		...
		echo $this->Paginator->next(__d('cake', 'próximo') .' >', array(), null, array('class' => 'next disabled'));
	?>
	</div>
</div>
<div class="actions">
	...
	<ul>
		<li><?php echo $this->Html->link(__d('cake', 'Novo %s', $singularHumanName), array('action' => 'add')); ?></li>
<?php
		$done = array();
